...
class StockMove(models.Model):
    ...
    quantity_available_loc_ids = fields.Many2many('sotck.quant', string = "Disponibles quants", compute = '_get_available')
    ...
    def _get_available(self):
        ...
        obj_product = self.env['product.product'].search([('product_tmpl_id', '=', self.product_id.id)])

        _logger.info("\nproduct_list: \n")
        _logger.info(obj_product)
        ...
        for i in obj_product:
            ...
            for obj in obj_quant:
                temp_line = {'product_id': obj.product_id.id,
                             'stock_location': obj.location_id.id,
                             'qty_on_hand': obj.qty,
                             }

                _logger.info(temp_line)

                product_list.append(temp_line)
                
            _logger.info("\n\nfor i in obj_product and for obj in obj_quant: running\n\n")
        _logger.info("\nproduct_list: \n")
        _logger.info(product_list)

        for i in product_list:

            _logger.info("\n\nfor i in product_list: running\n\n")
        ...
        for i in product_list:
            if i['qty_on_hand'] > 0:
                available_locations_ids.append(i['stock_location'].id)
                self.quantity_available_loc_ids = available_locations_ids#self.env['stock.quant'].browse([('location_id', 'in', available_locations_ids)])
